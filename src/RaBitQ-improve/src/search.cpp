...
void test(const Matrix<float> &Q, const Matrix<float> &RandQ,
  ...
  // ========================================================================
  // Search Parameter
  //   vector<int> nprobes = {20, 30, 40};
  vector<int> nprobes = {400};
  // vector<int> nprobes = {};
  ...
  for (auto nprobe : nprobes) {
    TopKBufferSoA upper_KNNs(k, NUM_BUCKETS, PSEUDO_BUCKETS);
    float total_time = 0;
    float total_ratio = 0;
    int correct = 0;
    rerank_count = 0;
    for (int i = 0; i < Q.n; i++) {
      ...
      while (p < result_ids.size() && q < k) {
        if (result_ids[p].second == G.data[i * G.d + q]) {
          ++tmp_correct;
          ++p;
          ++q;
        } else if (result_ids[p].second < G.data[i * G.d + q]) {
          ++p;
        } else {
          ++q;
        }
