...
void IVFRN<D, B>::load(char *filename)
{
    ...
    binary_code = static_cast<uint64_t *>(aligned_alloc(256, 1ull * N * B / 64 * sizeof(uint64_t)));
    ...
    for (int i = 0; i < C; i++)
    {
        pack_codes<B>(binary_code + 1ull * start[i] * (B / 64), len[i], packed_code + packed_start[i]);
    }
#else
    packed_start = NULL;
    packed_code = NULL;
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        fac[i].factor_ppc = -2 / fac_norm * x_x0 * ((float)space.popcount(binary_code + 1ull * i * B / 64) * 2 - B);
