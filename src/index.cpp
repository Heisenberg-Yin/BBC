...
int main(int argc, char * argv[]) {
    ...
    sprintf(dist_to_centroid_path, "%sdist_to_centroid_%d.fvecs", source, numC);
    Matrix<float> dist_to_centroid(dist_to_centroid_path);
    
    sprintf(cluster_id_path, "%scluster_id_%d.ivecs", source, numC);
    ...
    // ivf.save(index_path, X);
    
    ivf.save(index_path, X, C, dist_to_centroid, x0, cluster_id, binary);    
