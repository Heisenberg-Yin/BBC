...
public:
  ...
  void fast_scan(ResultHeap &KNNs, float &dist_to_centroid, float &distK,
                 uint32_t k, uint8_t *packed_code, uint32_t len, float *query,
                 float *data, uint32_t start_id);
  void compute_knn_core(const float *query, const float *data,
                        const uint32_t *id, size_t D, size_t k,
                        std::priority_queue<std::pair<float, uint32_t>>
                            &KNNs,         // 假设这是你的输入堆
                        ResultHeap &result // 输出结果
  );
  void load_candidate_from_disk(ResultHeap &KNNs,
                                std::vector<uint32_t> &candidate_ids);
  void rerank_disk(const float *query, std::vector<uint32_t> &candidate_ids,
                   size_t D, size_t k,
                   ResultHeap &result // 输出结果
  );
  ResultHeap search(float *query, float *rd_query, uint32_t num_cand,
                    uint32_t k, uint32_t nprobe,
                    float distK = std::numeric_limits<float>::max());
  ResultHeap search_on_disk(float *query, float *rd_query, uint32_t num_cand,
                            uint32_t k, uint32_t nprobe,
                            float distK = std::numeric_limits<float>::max());
...
void IVF_PQFastScan::compute_pq_LUT(float *query) {
...
void IVF_PQFastScan::fast_scan(ResultHeap &KNNs, float &dist_to_centroid,
                               float &distK, uint32_t k, uint8_t *packed_code,
...
__attribute__((noinline)) void IVF_PQFastScan::compute_knn_core(
    const float *query, const float *data, const uint32_t *id, size_t D,
    size_t k,
    std::priority_queue<std::pair<float, uint32_t>> &KNNs, // 假设这是你的输入堆
    ResultHeap &result                                     // 输出结果
) {
  float distK = std::numeric_limits<float>::max();
  uint32_t CL = (D * sizeof(float) + 63) / 64;
...
IVF_PQFastScan::rerank_disk(const float *query,
                            std::vector<uint32_t> &candidate_ids, size_t D,
                            size_t k,
                            ResultHeap &result // 输出结果
) {
  float distK = std::numeric_limits<float>::max();
  ...
  for (int i = 0; i < candidate_ids.size(); i++) {
    float dist = compute_l2_distance(query, data + 1ull * i * D, D);

    // Result update logic
    if (dist < distK) {
      result.emplace(dist, candidate_ids[i]);
      if (result.size() > k)
        result.pop();
      if (result.size() == k)
        distK = result.top().first;
...
void IVF_PQFastScan::load_candidate_from_disk(
    ResultHeap &KNNs, std::vector<uint32_t> &candidate_ids) {
  while (!KNNs.empty()) {
    candidate_ids.push_back(*(id + KNNs.top().second));
    KNNs.pop();
  }
  //   std::sort(candidate_ids.begin(), candidate_ids.end());
  v_map->fetch_vectors(candidate_ids, data);
}

ResultHeap IVF_PQFastScan::search(float *query, float *rd_query,
                                  uint32_t num_cand, uint32_t k,
                                  uint32_t nprobe, float distK) {
  ResultHeap KNNs;
  ...
  for (int pb = 0; pb < nprobe; pb++) {
    ...
    fast_scan(KNNs, dist_to_centroid, distK, num_cand, ptr_packed_code, len[c],
              query, ptr_data, start[c]);
    ptr_centroid_dist++;
  }
  ResultHeap result;
  distK = std::numeric_limits<float>::max();
  uint32_t CL = (D * sizeof(float) + 63) / 64; // 每条向量多少行 64B
  compute_knn_core(query, data, id, D, k, KNNs, result);
  //   while (!KNNs.empty()) {
  //     uint32_t idx = KNNs.top().second;
  //     KNNs.pop();
  //     if (!KNNs.empty()) {
...
ResultHeap IVF_PQFastScan::search_on_disk(float *query, float *rd_query,
                                          uint32_t num_cand, uint32_t k,
                                          uint32_t nprobe, float distK) {
  ResultHeap KNNs;
  Result centroid_dist[C];
  ...
  candidate_ids.reserve(KNNs.size());
  load_candidate_from_disk(KNNs, candidate_ids);
  ResultHeap result;
  rerank_disk(query, candidate_ids, D, k, result);
  //   compute_knn_core(query, data, id, D, k, KNNs, result);
  //   while (!KNNs.empty()) {
  //     uint32_t idx = KNNs.top().second;
  //     KNNs.pop();
  //     result.emplace(1, *(id + idx));
  //   }
  return result;
}
...
void IVF_PQFastScan::load_disk(char *filename, unsigned num_cand) {
  ...
  data = new float[1ull * num_cand * D];
  //   size_t alignment = 4096;
  //   size_t total_bytes = 1ull * num_cand * D * sizeof(float);

  //   // 确保 size 是 alignment 的倍数
  //   size_t rounded_size = (total_bytes + alignment - 1) & ~(alignment - 1);

  //   float *data =
  //       static_cast<float *>(std::aligned_alloc(alignment, rounded_size));

  v_map = new AsyncVectorMap("/yinziqi/marco-30m/base.fvecs", D);
  std::cerr << "loaded!" << std::endl;
